...
export default async function Storygram({ searchParams }) {
  ...
  const sortedStorygrams = storygrams.sort((a, b) => new Date(b.date) - new Date(a.date));

  const startIndex = (currentPage - 1) * pageSize;
  const paginatedStorygrams = sortedStorygrams.slice(startIndex, startIndex + pageSize);
  ...
  return (
    <main className="bg-darker">
      <div className="mx-auto max-w-5xl pt-32 text-white">
        ...
        <Pagination totalPages={totalPages} currentPage={currentPage} url={"storygram"}/>
      </div>
    </main>
  );
}
