...
const Header = ({ navbarLinks, socialLinks, siteLogo }) => {
  ...
  return (
    <header className="z-50 relative" ref={headerRef}>
      ...
      <div className="block md:hidden bg-[#161616]">
        <div className="flex justify-between items-center py-6 px-8">
          ...
          <button onClick={() => setMenuOpen((menuOpen) => !menuOpen)}>
            <div className={`w-7 h-5 relative`}>
              <div
                className={`absolute w-full h-1 bg-white transform origin-center ${
                  menuOpen ? "top-1/2 -translate-y-1/2 rotate-45" : "top-0"
                }`}
              />
              <div
                className={`absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 h-1 bg-white ${
                  menuOpen ? "w-0" : "w-full"
                }`}
              />
              <div
